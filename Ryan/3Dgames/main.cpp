...
Ogre::Root*                 OgrRoot;
Ogre::RenderWindow*			OgrRenderWnd;
Ogre::Viewport*				OgrViewport;
Ogre::Log*				    OgrLog;
Ogre::Timer*				OgrTimer;
...
void InitializeOgre(){
    int viewW, viewH; SDL_GetWindowSize(portals[0]->viewPorts->window, &viewW, &viewH);

    OgrRoot = new Ogre::Root("plugins.cfg", "ogre.cfg", "ogre.log");

    if(!(OgrRoot->restoreConfig() || OgrRoot->showConfigDialog())) {MSG("User Exited dialog\n"); exit(2);}

    OgrRoot->initialise(false, "RenderWnd");

    Ogre::NameValuePairList misc;  misc["currentGLContext"] = Ogre::String("True");
    OgrRenderWnd = OgrRoot->createRenderWindow("MainRenderWindow", viewW, viewH, false, &misc);
    OgrRenderWnd->setVisible(true);
	OgrRenderWnd->setAutoUpdated(false);  // Don't automatically swap video buffers

    setupResources();  // Define Resource Groups
    MainSceneMgr = OgrRoot->createSceneManager(Ogre::ST_GENERIC, "MySceneMgr");
    Ogre::OverlaySystem * overlaySystem = new Ogre::OverlaySystem; MainSceneMgr->addRenderQueueListener( overlaySystem );
...
void InitializePortalSystem(int argc, char** argv){
    ...
    CreateTurbulancePortal(AppWindowTitle.data(), 2000, 100 /*1200*/,1200,700);
}
