...
Matrix2& Matrix2::invert()
{
    ...
    scalar tmp = m[0];   // copy the first element
...
{
    scalar determinant, invDeterminant;
    scalar tmp[9];
...
Matrix4& Matrix4::scale(scalar x, scalar y, scalar z)
{
...
{
    scalar c = cosf(angle * DEG2RAD);    // cosine
    scalar s = sinf(angle * DEG2RAD);    // sine
    scalar c1 = 1.0f - c;                // 1 - c
    scalar m0 = m[0],  m4 = m[4],  m8 = m[8],  m12= m[12],
          m1 = m[1],  m5 = m[5],  m9 = m[9],  m13= m[13],
          m2 = m[2],  m6 = m[6],  m10= m[10], m14= m[14];

    // build rotation matrix
    scalar r0 = x * x * c1 + c;
    scalar r1 = x * y * c1 + z * s;
    scalar r2 = x * z * c1 - y * s;
    scalar r4 = x * y * c1 - z * s;
    scalar r5 = y * y * c1 + c;
    scalar r6 = y * z * c1 + x * s;
    scalar r8 = x * z * c1 + y * s;
    scalar r9 = y * z * c1 - x * s;
    scalar r10= z * z * c1 + c;
...
{
    scalar c = cosf(angle * DEG2RAD);
    scalar s = sinf(angle * DEG2RAD);
    scalar m1 = m[1],  m2 = m[2],
          m5 = m[5],  m6 = m[6],
          m9 = m[9],  m10= m[10],
          m13= m[13], m14= m[14];
